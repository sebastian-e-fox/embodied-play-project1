...
namespace Tobii.Gaming.Examples.ActionGame
{
	public class CleanUIForCanvas : CleanUI
	{
		...
		protected override Rect GetViewportBounds()
		{
			...
			return CalculateViewportBounds(GetComponentInParent<Canvas>(), AdditionalEyetrackingMarginScale,
				areaWorldSpaceMin, areaWorldSpaceMax);
		}
		...
		public static Rect CalculateCompositeBounds(params GameObject[] objects)
		{
			...
			return CalculateViewportBounds(objects[0].GetComponentInParent<Canvas>(), 0.3f, areaWorldSpaceMin,
				areaWorldSpaceMax);
		}

		private static Rect CalculateViewportBounds(Canvas canvas, float additionalEyetrackingMarginScale,
			Vector2 areaWorldSpaceMin, Vector2 areaWorldSpaceMax)
		{
			var unadjustedBounds = new Rect(areaWorldSpaceMin.x, areaWorldSpaceMin.y,
				areaWorldSpaceMax.x - areaWorldSpaceMin.x, areaWorldSpaceMax.y - areaWorldSpaceMin.y);
			...
			if (canvas != null)
			{
				if (canvas.worldCamera != null)
				{
					...
					var rightBottom =
						renderingCamera.WorldToScreenPoint(unadjustedBounds.position + unadjustedBounds.size);
			...
			var pixelScale = Mathf.Min(unadjustedBounds.width, unadjustedBounds.height) *
			                 additionalEyetrackingMarginScale;

			return new Rect((unadjustedBounds.position.x - pixelScale) / Screen.width,
				(unadjustedBounds.position.y - pixelScale) / Screen.height,
				(unadjustedBounds.width + pixelScale * 2.0f) / Screen.width,
				(unadjustedBounds.height + pixelScale * 2.0f) / Screen.height);
		}
		...
		private static Vector4 RectTransformBounds(RectTransform rt)
		{
			...
			returnVector.x = Mathf.Min(Mathf.Min(Mathf.Min(fourCorners[0].x, fourCorners[1].x), fourCorners[2].x),
				fourCorners[3].x);
			returnVector.y = Mathf.Min(Mathf.Min(Mathf.Min(fourCorners[0].y, fourCorners[1].y), fourCorners[2].y),
				fourCorners[3].y);
			returnVector.z = Mathf.Max(Mathf.Max(Mathf.Max(fourCorners[0].x, fourCorners[1].x), fourCorners[2].x),
				fourCorners[3].x);
			returnVector.w = Mathf.Max(Mathf.Max(Mathf.Max(fourCorners[0].y, fourCorners[1].y), fourCorners[2].y),
				fourCorners[3].y);
			return returnVector;
		}
	}
