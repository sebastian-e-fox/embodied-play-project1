 ...
 */
	...
	public abstract class CleanUI : MonoBehaviour
	{
		...
		public static ReadOnlyCollection<CleanUI> GetCleanUIElements()
		{
			return _cleanUIList.AsReadOnly();
		}
		...
		protected void Update()
		{
			...
			{
				_currentOpacityInput =
					Mathf.Clamp01(_currentOpacityInput + Time.unscaledDeltaTime * (1.0f / FadeInTimeSecs));
			}
			else
			{
				_currentOpacityInput =
					Mathf.Clamp01(_currentOpacityInput - Time.unscaledDeltaTime * (1.0f / FadeOutTimeSecs));
			}
			...
			for (var index = 0; index < UIElements.Count; ++index)
			{
				...
				if (
					element == null) //If it has been deleted, we need to remove our reference to allow GC. Unity overloads == to allow us to test like this
				{
