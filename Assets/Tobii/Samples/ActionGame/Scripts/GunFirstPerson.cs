...
namespace Tobii.Gaming.Examples.ActionGame
{
	public class GunFirstPerson : Gun
	{
		protected override void AlignGunToCrosshairDirection()
		{
			...
			{
				var laserSightDesiredHeading =
					WeaponController.WeaponHitData.point - OptionalLaserSight.transform.position;
				laserSightDesiredHeading.Normalize();
				transform.rotation = Quaternion.Lerp(transform.rotation,
					Quaternion.LookRotation(laserSightDesiredHeading), GunAlignmentSpeed);
			}
			else
			{
				var desiredHeading = WeaponController.WeaponHitData.point - transform.position;
				desiredHeading.Normalize();
				transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(desiredHeading),
					GunAlignmentSpeed);
			}
		}
	}
