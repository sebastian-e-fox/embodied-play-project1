...
namespace Tobii.Gaming.Examples.MapNavigation
{
	public class MapNavigation : MapNavigationBase
	{
		...
		{
			get { return GetCurrentGroundPositionAtScreenCenter() - CameraTransform.position; }
		}
		...
		protected override bool ZoomOut(float currentZoomLevel, float targetZoomLevel)
		{
			...
			if (currentZoomLevel < targetZoomLevel - zoomStepAmount)
			{
				...
				CameraTransform.position = GetCurrentGroundPositionAtScreenCenter() -
				                           Vector3.Normalize(ZoomVector) * currentZoomLevel;
			}
			else
			{
